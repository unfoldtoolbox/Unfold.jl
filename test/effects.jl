...
@testset "MixedModel" begin
	...
	m = fit(UnfoldModel,@formula(0 ~ 1+condition + (1+condition|subject)),evts,data,1:size(data,1))
...
@testset "MixedModelContinuousTime" begin
	...
	m = fit(UnfoldModel,Dict(Any=>(@formula(0 ~ 1+condition + zerocorr(1+condition|subject)),firbasis([0,1],10))),evts,data)
