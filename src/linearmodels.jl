...
struct UnfoldMixedModelFitCollection{T<:AbstractFloat} <: MixedModels.MixedModelFitCollection{T}
    fits::Vector
...
mutable struct UnfoldLinearMixedModel <:UnfoldModel
    design::Dict
    designmatrix::DesignMatrix
    modelfit#::Array{UnfoldMixedModelFitCollection} # optional info on the modelfit
end
...
UnfoldLinearModelContinuousTime(d::Dict) = UnfoldLinearModelContinuousTime(d,Unfold.DesignMatrix(),[])
UnfoldLinearModelContinuousTime(d::Dict,X::DesignMatrix) = UnfoldLinearModelContinuousTime(d,X,[])

mutable struct UnfoldLinearMixedModelContinuousTime  <:UnfoldModel
    design::Dict
    designmatrix::DesignMatrix
    modelfit#::UnfoldMixedModelFitCollection
end

UnfoldLinearMixedModelContinuousTime(d::Dict) = UnfoldLinearMixedModelContinuousTime(d,Unfold.DesignMatrix(),[])
UnfoldLinearMixedModelContinuousTime(d::Dict,X::DesignMatrix) = UnfoldLinearMixedModelContinuousTime(d,X,[])
